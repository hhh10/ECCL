...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        super().__init__()
        if design_details['lora']:
            self.attn = lora.MultiheadAttention(d_model, n_head, r=4, lora_alpha=design_details["lora_alpha"])
        else:
            self.attn = nn.MultiheadAttention(d_model, n_head)
        ...
        #######Modify add lora######
        # if design_details['lora_FFN']:
        rank=4
        self.mlp = nn.Sequential(OrderedDict([

            ("c_fc", lora.Linear(d_model, d_model * 4, r=rank, lora_alpha=design_details["lora_alpha"])),
            ("gelu", QuickGELU()),
            ("c_proj", lora.Linear(d_model * 4, d_model, r=rank, lora_alpha=design_details["lora_alpha"]))
        ]))
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, stride_size: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution

        # person reid modification
        ...
        # modify positional embedding length same to reid image resolution
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                self.visual = VisionTransformer(
                    input_resolution=image_resolution,
                    stride_size=stride_size,  # modify
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
                    heads=vision_heads,
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        if isinstance(l, (nn.Conv1d, nn.Conv2d, nn.Linear, lora.Linear)):
...
def build_model(image_size: Union[int, Tuple[int, int]], stride_size: int, state_dict: dict, design_details):
    ...
    model = CLIP(
        embed_dim,
        image_resolution, stride_size, vision_layers, vision_width, vision_patch_size,
    ...
    #         v = resize_pos_embed(v,model.visual.positional_embedding,24,8)

    visual_pos_embed = state_dict['visual.positional_embedding']
    visual_pos_embed = resize_pos_embed(visual_pos_embed, model.visual.positional_embedding, 24, 8)
    state_dict['visual.positional_embedding'] = visual_pos_embed
