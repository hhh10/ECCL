...
def do_train(start_epoch, args, model, train_loader, evaluator, optimizer,
             scheduler, checkpointer, num_classes,num_instance):
    ...
    if 'instance_memory' in args.loss_names:
        ...
        instance_memory_image = InstanceMemory(
            ...
            momentum=args.instance_momentum,
            margin=args.margin
        ).to(device)
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        for n_iter, batch in enumerate(train_loader):
            ...
            total_loss = sum([v.float() for k, v in ret.items() if "loss" in k])
        ...
        if get_rank() == 0:
            ...
            logger.info(
                "Epoch {} done. Time per batch: {:.3f}[s] Speed: {:.1f}[samples/s] Epoch time: {:.3f}[s]"
                .format(epoch, time_per_batch,
                        train_loader.batch_size / time_per_batch, epoch_time))
