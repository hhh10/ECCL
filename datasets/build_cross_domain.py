...
def build_dataloader(args, tranforms=None):
    ...
    # num_classes = len(dataset.train_id_container)
    # num_instance = len(dataset.train_instance_id_container)
    if args.training:
        ...
        return train_loader, val_img_loader, val_txt_loader, num_classes, num_instance
    else:
...
def build_zero_shot_loader(args, finetune=False):
    ...
    num_classes = len(dataset2.train_id_container)
    num_instance = len(dataset2.train_instance_id_container)
    train_transforms = build_transforms(img_size=args.img_size,
                                            aug=args.img_aug,
                                            is_train=True)
    val_transforms = build_transforms(img_size=args.img_size,
    ...
    if finetune:
        ...
        syn_dataset = __factory[args.pretrain](root=args.root_dir)
    train_set = ImageTextDataset(syn_dataset.train,
                            train_transforms,
                            text_length=args.text_length)
    # train_set = ImageTextMLMDataset(syn_dataset.train,
    #                         train_transforms,
    ...
    num_instance = len(syn_dataset.train_instance_id_container)
    logger.info('using random sampler')
    train_loader = DataLoader(train_set,
                                batch_size=args.batch_size,
                                shuffle=True,
    ...
    return syn_dataset.train, train_loader, val_img_loader0, val_txt_loader0, val_img_loader1, val_txt_loader1, val_img_loader2, val_txt_loader2, num_classes, num_instance
