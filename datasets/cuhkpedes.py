...
class CUHKPEDES(BaseDataset):
    ...
    def __init__(self, root='', verbose=True):
        ...
        # self.train_anno_path = '/data/hyj/annotation/CUHK-PEDES/train_reid.json'
        # self.train_annos = read_json(self.train_anno_path)

        self.train, self.train_id_container, self.train_instance_id_container = self._process_anno(self.train_annos, training=True)
    ...
    def _process_anno(self, annos: List[dict], training=False):
        ...
        if training:
            ...
            for anno in annos:
                ...
                instance_id_container.add(image_id)
                img_path = op.join(self.img_dir, anno['file_path'])
                captions = anno['captions'] # caption list
                # # Add back translation caption
                # captions_bt = anno['captions_bt']
                # for caption, captions_bt in zip(captions, captions_bt):
                #     dataset.append((pid, image_id, img_path, caption, captions_bt))
            ...
            return dataset, pid_container, instance_id_container
        else:
            dataset = {}
            img_paths = []
            captions = []
